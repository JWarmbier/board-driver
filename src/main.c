...
#define TRAJECTORY_THREAD_STACK_SIZE 512
#define TRAJECTORY_THREAD_PRIORITY 4
...
#define CAN_ID_SILNIK1 0x581
#define CAN_ID_SILNIK2 0x582
...
K_THREAD_STACK_DEFINE(trajectory_thread_stack, TRAJECTORY_THREAD_STACK_SIZE);
...
struct k_thread trajectory_thread_data;
...
void trajectory_thread(void *arg1, void *arg2, void *arg3){
	...
	uint32_t cnt = 0;
	/*motor_set_velocity(motor, can_dev,100);
	motor_enable(motor,can_dev);*/
...
//	gpio_pin_configure(dev, LED, GPIO_DIR_OUT);
	...
	const struct zcan_filter change_led_filter = {
		...
		k_tid_t rx_tid, get_state_tid, trajectory_tid;
		int ret;
